    ...
    for (let i = 0; i < checkboxs.length; i++) {
        checkboxs[i].addEventListener('click', () => {
            for (let j = 0; j < checkboxs.length; j++) { //将所有按钮设置为空
                checkboxs[j].children[0].children[0].src = '../images1/checkbox_none.svg';
                checkboxs[j].style.fontSize = '16px';
            }
            //设置当前点中的复选框为选中状态
            checkboxs[i].children[0].children[0].src = '../images1/checkbox_selected.svg';
...
function selectType(checkboxs) {
    ...
    for (let i = 0; i < checkboxs.length; i++) {
        checkboxs[i].addEventListener('click', () => {
            for (let j = 0; j < checkboxs.length; j++) { //将所有按钮设置为空
                checkboxs[j].children[0].children[0].src = '../images1/private.svg';
                checkboxs[j].style.fontSize = '20px';
            }
            //设置当前点中的复选框为选中状态
            checkboxs[i].children[0].children[0].src = '../images1/public.svg';
            ...
            success: function(result) { //返回接受信息
                    ...
                    ajax({
                        ...
                        success: function(result) { //返回接受信息
                            if (result.message == 'success') {
                                icon.children[0].src = '../images1/starToConcern.png';
                                btn.innerHTML = '+ 关注';
                ...
                if (result.data.hasMark == false) { //如果未关注，则关注
                    ajax({
                        ...
                        success: function(result) { //返回接受信息
                            if (result.message == 'success') {
                                icon.children[0].src = '../images1/starToConcerned.png';
                                btn.innerHTML = '已关注';
                                people.children[1].innerHTML = result.data.markCount;
                            }
                        }
                    ...
                    ajax({
                        ...
                        success: function(results) { //返回接受信息
                            if (results.message == 'success') {
                                ...
                                btn.children[0].src = '../images1/hug.png';
                            }
                        }
                    });
                }
                if (results.data.result == false) { //如果未点赞，则点赞
                    ajax({
                        ...
                        success: function(results) { //返回接受信息
                            if (results.message == 'success') {
                                icon.innerHTML = results.data.likeCount;
                                btn.children[0].src = '../images1/btn_hugged.png';
                            }
                        }
                    });
                }
